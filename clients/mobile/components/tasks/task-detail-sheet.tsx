...
import { TASK_ASSIGNMENT_STATE, type TaskAssignmentState } from "@/constants/tasks";
...
export const TaskDetailSheet = forwardRef<BottomSheetModal, TaskDetailSheetProps>(
  function TaskDetailSheet(
    { task, listVariant, onDismiss, onStart, onClaim, onMarkDone, onDrop, onFindCover },
    ref,
  ) {
          ...
          {!task ? (
            ...
            <>
              ...
              <View className="mt-3 gap-1">
                ...
                <Text className="text-sm text-gray-700">
                  <Text className="text-gray-500">Department:</Text> {task.department}
                </Text>
                <Text className="text-sm text-gray-700">
              ...
              {task.description ? (
                <Text className="text-sm text-gray-600 mt-4">{task.description}</Text>
              ) : null}
              ...
              <View className="mt-6 gap-3">
                {assigned ? (
                  <>
                    ...
                    {st === "assigned" ? (
                      <Pressable onPress={() => onFindCover(task)} className="py-2">
                        <Text className="text-center text-sm" style={{ color: primary }}>
                          Find a Cover
                        </Text>
                      </Pressable>
                    ) : null}
                    ...
                    {st === "in progress" ? (
                      <Pressable onPress={() => onDrop(task)} className="py-2">
                        <Text className="text-center text-sm text-red-600">Drop task</Text>
                      </Pressable>
                    ) : null}
              ...
              <View className="border border-[#E5E9ED] rounded-lg p-3">
                <Text className="text-sm text-gray-500">Activity log coming soon.</Text>
              </View>
            </>
          )}
        </BottomSheetScrollView>
