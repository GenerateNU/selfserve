...
function StaysCollapsible({ title, stays, emptyMessage }: { title: string; stays: Stay[]; emptyMessage: string }) {
    return (
...
function GuestInfoCollapsibles(props: GuestProfileProps) {
    return (
        <Box className="p-[4vw] gap-[2vh]">
            ...
            <StaysCollapsible title="Current Stays" stays={props.currentStays} emptyMessage="No current stays" />
            <StaysCollapsible title="Previous Stays" stays={props.previousStays} emptyMessage="No previous stays" />
        </Box>
    );
}
