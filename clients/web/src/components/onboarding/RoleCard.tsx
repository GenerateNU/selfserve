...
export function RoleCard({ label, description, selected, onSelect }: RoleCardProps) {
  return (
    <button
      ...
      <p style={{ fontSize: "clamp(12px, 1.2vw, 16px)", fontWeight: 500 }} className="text-gray-900 m-0">{label}</p>
      <p style={{ fontSize: "clamp(10px, 1vw, 14px)" }} className="text-gray-500 mt-1 m-0">{description}</p>
    </button>
  );
}