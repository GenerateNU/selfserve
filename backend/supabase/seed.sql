...
-- -----------------------------------------------------------------------------
-- Requests — 30 detailed rows: 10 pending · 10 assigned · 10 completed
-- request_version set to 2026-04-01 00:01–00:30 so they sort first in kanban.
-- All fields populated with realistic hotel scenarios.
-- -----------------------------------------------------------------------------
...
-- request_version starts at 2026-04-02 so detailed rows sort first in kanban.
-- -----------------------------------------------------------------------------
...
SELECT
  ...
  (ARRAY['recurring','on demand'])[((i-1) % 2) + 1],
  'pending',
  (ARRAY['low','medium','high'])[((i-1) % 3) + 1],
  (ARRAY['10000000-0000-0000-0000-000000000101','10000000-0000-0000-0000-000000000102',
         '10000000-0000-0000-0000-000000000103','10000000-0000-0000-0000-000000000201',
         '10000000-0000-0000-0000-000000000202','10000000-0000-0000-0000-000000000203',
         '10000000-0000-0000-0000-000000000301','10000000-0000-0000-0000-000000000302',
         '10000000-0000-0000-0000-000000000303'])[((i-1) % 9) + 1],
  (ARRAY['Housekeeping','Maintenance','Concierge','Food & Beverage','Front Desk'])[((i-1) % 5) + 1],
  '2026-04-02'::date + '08:00:00'::interval + (((i-1) % 12) || ' hours')::interval,
  '2026-04-02'::timestamptz + (i || ' hours')::interval,
  '2026-04-02'::timestamptz + (i || ' hours')::interval
...
SELECT
  ...
  (ARRAY['Housekeeping','Maintenance','Concierge','Food & Beverage','Front Desk'])[((i-1) % 5) + 1],
  '2026-04-02'::date + '08:00:00'::interval + (((i-1) % 12) || ' hours')::interval,
  '2026-04-03'::timestamptz + (i || ' hours')::interval,
  '2026-04-03'::timestamptz + (i || ' hours')::interval
FROM generate_series(1, 100) AS i
ON CONFLICT (id, request_version) DO NOTHING;
...
SELECT
  ...
  (ARRAY['Housekeeping','Maintenance','Concierge','Food & Beverage','Front Desk'])[((i-1) % 5) + 1],
  '2026-04-02'::date + '08:00:00'::interval + (((i-1) % 12) || ' hours')::interval,
  '2026-04-04'::timestamptz + (i || ' hours')::interval,
  '2026-04-04'::timestamptz + (i || ' hours')::interval
FROM generate_series(1, 100) AS i
ON CONFLICT (id, request_version) DO NOTHING;
